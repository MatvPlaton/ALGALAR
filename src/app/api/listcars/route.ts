...
export async function GET() {
  try {
    ...
    let response = await fetch(`${process.env.BACKEND_URL}/positions/listcars?limit=100&offset=0`, {
      method: 'GET',
      headers: {
    ...
    if (response.status === 401 && refreshToken) {
      ...
      const newTokens = await refreshAccessToken(refreshToken.value);
        ...
        response = await fetch(`${process.env.BACKEND_URL}/positions/listcars?limit=100&offset=0`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
