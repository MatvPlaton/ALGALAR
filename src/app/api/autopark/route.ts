...
export async function GET() {
  try {
    ...
    let response = await fetch(`${process.env.BACKEND_URL}/auto/list?offset=0&limit=100`, {
      method: 'GET',
      headers: {
    ...
    if (response.status === 401 && refreshToken) {
      ...
      const newTokens = await refreshAccessToken(refreshToken.value);
        ...
        response = await fetch(`${process.env.BACKEND_URL}/auto/list?offset=0&limit=100`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
