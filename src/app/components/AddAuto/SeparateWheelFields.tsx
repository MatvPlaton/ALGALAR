...
const SeparateWheelFields: React.FC<Prop> = ({carId, currTire, setImages, setRedacting, redacting}) => {
    ...
    const [wheels, setWheels] = useState<Stats[]>(Array(24).fill(
        {
            number: '',
            size : '',
            ngp: '',
    ...
    const send  = () => {
        ...
        if (wheels[currTire]['number'] === '') { setNumberTurn(true); flag = true }
        if (wheels[currTire]['size'] === '') { setSizeTurn(true); flag = true }
        if (wheels[currTire]['ngp'] === '') { setNgpTurn(true); flag = true }
        if (wheels[currTire]['tkvh'] === '') { setTkvhTurn(true); flag = true }
        if (wheels[currTire]['brand'] === '') { setBrandTurn(true); flag = true }
        if (wheels[currTire]['cost'] === '') { setCostTurn(true); flag = true }
        if (wheels[currTire]['model'] === '') { setModelTurn(true); flag = true }
        if (flag) return;
    ...
    return <> {redacting ? <>
        ...
        <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'number'} index={currTire} turn={numberTurn} setTurn={setNumberTurn} text={'Номер датчика*'} />
        <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'size'} index={currTire} turn={sizeTurn} setTurn={setSizeTurn} text={'Размер шины*'} />
            <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'ngp'} index={currTire} turn={ngpTurn} setTurn={setNgpTurn} text={'НГП*'} />
            <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'model'} index={currTire} turn={modelTurn} setTurn={setModelTurn} text={'Модель шины*'} />

        </SeparateWheelFieldsWrapper>
        <SeparateWheelFieldsWrapper>
            <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'brand'} index={currTire} turn={brandTurn} setTurn={setBrandTurn} text={'Бренд шины*'} />
            <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'cost'} index={currTire} turn={costTurn} setTurn={setCostTurn} text={'Стоимость шины(тыс. руб.)*'} />
            <AnyFieldWheel wheels={wheels} setWheels={setWheels} field={'tkvh'} index={currTire} turn={tkvhTurn} setTurn={setTkvhTurn} text={'ТКВЧ*'} />
        </SeparateWheelFieldsWrapper>

        </FieldsWrapper>

            <TempAndPressure wheels={wheels} currTire={currTire} setWheels={setWheels} redacting={redacting} />
