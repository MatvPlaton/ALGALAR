...
import {
  ALGALAR,
  Description,
} from '@/app/components/Profile/styles/ProfileMenu';
    ...
    <SidebarWrapper height={height} $active={isExpanded}>
      ...
      <ComponentsList>
        {Object.keys(Pictures).map((key: string) => (
          <Component
            onClick={() => router.push(`/${Pictures[key][2]}`)}
            key={key}
          >
            {activeField === key ? (
              <>
      ...
      <ToggleButton $active={isExpanded} onClick={toggleSidebar}>
        {isExpanded ? (
          <Image style={{ width: '2vw', height: '4vh' }} src={arrow1} alt="" />
        ) : (
          <Image style={{ width: '2vw', height: '4vh' }} src={arrow2} alt="" />
        )}{' '}
      </ToggleButton>
    </SidebarWrapper>
  );
