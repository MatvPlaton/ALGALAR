    ...
    return (
        ...
        h-[100vh] items-center`}>
            <div className={` absolute h-[10vh] font-[InterExtraBold] text-white left-[2.3%] w-[17vw] flex`}>
                ...
                <div className={`relative left-[4.2%] top-[45%] text-[0.6vw]`}> техника безопасного вождения </div>
            </div>
            <div className="absolute flex flex-col top-[10vh] w-[26.5vw] h-4/5 left-[5%]">
                {Object.keys(Pictures).map((key : string) =>

                    <div className="cursor-pointer relative flex h-[15%] flex-row w-full"
                    onClick={() => key === 'Exit' ? exitSession() : router.push( `/${key}`)} key={key}>
                    
                    {activeField === key ? 
                    <Image className="absolute top-[-15%] left-[-0.2%] w-full h-[4.2vw]" 
                    src={Pictures[key][1]} alt={''} /> : 
                    <Image className="absolute left-[3.2%] w-[2.5vw] h-[2.5vw]" 
                    src={Pictures[key][0]} alt={''} />
                }
                    
                    <div className={`relative left-[23%] top-[5%] font-[RobotoRegular] text-[1.4vw] text-${activeField === key ? 'black' : 'white'}`}> {Pictures[key][2]} </div>
                    </div>)}
            </div>


                <Image style={{ left: isExpanded ? "86%" : "30%" }} className={`transition-all duration-300 ease-in-out
                absolute top-9/10 w-[2vw] h-[4vh]`} 
                onClick={toggleSidebar} src={isExpanded ? arrow1 : arrow2} alt='' />
                </div>
)
